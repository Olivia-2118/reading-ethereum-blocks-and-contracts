...
def connect_to_eth():
  ...
  url = "https://mainnet.infura.io/v3/aeb66997503f4dbba46cbbcf230edfc6"  # Infura URL in last week assignment
...
def is_ordered_block(w3, block_num):
  ...
  base_fee = block.get("baseFeePerGas", None)
	...
	# 0/1 transactions are treated as ordered 
	if len(txs) <= 1:
		return True

	def priority_fee(tx) -> int:
		# before EIP:1559：only count gasPrice
		if base_fee is None:
			return int(tx.get("gasPrice", 0))

		# after EIP-1559：
		# Type 2：there are maxPriorityFeePerGas & maxFeePerGas
		mp = tx.get("maxPriorityFeePerGas")
		mf = tx.get("maxFeePerGas")
		if mp is not None and mf is not None:
			# min( mp, mf - baseFee )
			return max(min(int(mp), int(mf) - int(base_fee)), 0)

		# Type 0：priority = gasPrice - baseFee
		gp = tx.get("gasPrice", 0)
		return max(int(gp) - int(base_fee), 0)

	prios = [priority_fee(tx) for tx in txs]
	ordered = all(prios[i] >= prios[i + 1] for i in range(len(prios) - 1))
